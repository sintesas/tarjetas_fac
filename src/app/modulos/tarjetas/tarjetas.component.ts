...
export class TarjetasComponent {
    ...
    if (esEncontrado.length == 0) {
      this.apiT.Obtenerdatos({ id: num }).subscribe(data => {
        ...
        if (response.tipo == 0 && response.result[0] != null) {
          ...
          avatarImg.src = this.apiT.apiGetNombreFoto + "?identificacion=" + num;
          avatarImg.onload = () => {
          ...
          this.apiT.ObtenerViewData({ identificacion: num }).subscribe(data1 => {
            let response1: any = this.api.ProcesarRespuesta(data1);
            if (response1.tipo == 0 && response1.result != null) {
              response1.result.Unidad = response1.result.dependencia1;
              response1.result.Dependencia = response1.result.dependencia;
              let datos = response1.result;
  ...
  CrearPersona(datos: any, num: number) {
    ...
    let usuarioDA = ""
    this.apiT.ObtenerUsuarioDA({ identificacion: num }).subscribe(data9 => {
      let response9: any = this.api.ProcesarRespuesta(data9);
      if (response9.tipo == 0 && response9.result != null) {
        usuarioDA = response9.result.usuario_da;
      }
    })
  ...
  editar(datos: any) {
    ...
    if (datos.existe_img == 1) {
      ...
      const avatarImg = new Image();
      avatarImg.src = this.apiT.apiGetNombreFoto + "?identificacion=" + datos.numero_identificacion;
      avatarImg.onload = () => {
        this.ctx.clearRect(0, 0, this.canvas.nativeElement.width, this.canvas.nativeElement.height);
        this.ctx.drawImage(avatarImg, 0, 0, this.canvas.nativeElement.width, this.canvas.nativeElement.height);
      }
      ...
      //             Swal.fire({
      //               title: 'Error',
      //               text: 'Imagen no disponible. Si desea cargar una imagen por favor guárdela en la unidad Z con el siguiente nombre: ' + this.nombrefoto + ' o continue con el proceso.',
      //               icon: 'error',
      //               allowOutsideClick: false,
      //               showCancelButton: false,
      //               showConfirmButton: true,
      ...
      //         Swal.fire({
      //           title: 'Error',
      //           text: 'Imagen no disponible. Si desea cargar una imagen por favor dirijase al módulo Personas o continue con el proceso.',
      //           icon: 'error',
      //           allowOutsideClick: false,
      //           showCancelButton: false,
      //           showConfirmButton: true,
  ...
  buscarFoto() {
    ...
    this.apiT.ObtenerNombreImagen({ numero_identificacion: this.model.varTarjeta.numero_identificacion }).subscribe(
      (data: any) => {
        try {
          if (data.imagen) {
            this.imagen = data.imagen;
            console.log("Nombre de la foto:", this.imagen);
            if (this.imagen !== 'null') {
              this.nombrefoto = 'null';
              this.cargarImagen();
            }
          } else if (this.nombrefoto !== 'null' && this.imagen === 'null') {
            this.imagen = 'null';
            this.cargarFoto();

          } else {
            if (this.nombrefoto === 'null' && this.imagen === 'null') {
              this.nombrefoto = 'null';
              this.imagen = 'null';
              this.cargarFoto();
            }
  ...
  cargarImagen() {
    ...
    if (this.imagen !== 'null') {
      let url = this.api.URL;
      // Construye la ruta completa a la imagen en la carpeta assets/images
      const rutaBase = this.api.imagen_folder;
      this.rutaImagen = `${rutaBase}${this.imagen}`;
      console.log(url);
      const img = new Image();
      img.onload = () => {
        this.ctx.clearRect(0, 0, this.canvas.nativeElement.width, this.canvas.nativeElement.height);
      ...
      this.showError = true;
      console.log('La foto no se encuentra en la carpeta perfil');
      this.rutaImagen = "../../../assets/images/avatar.jpg";
      const avatarImg = new Image();
      avatarImg.onload = () => {
        this.ctx.clearRect(0, 0, this.canvas.nativeElement.width, this.canvas.nativeElement.height);
